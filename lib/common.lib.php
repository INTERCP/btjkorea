...
function get_file($bo_table, $wr_id)
{
    ...
    while ($row = sql_fetch_array($result))
    {
		...
		// 이전한 DB 라면
		if( $row['bf_source'] == $row['bf_file'] ){
			 $file[$no]['path'] = G5_DATA_URL.'/upload';
		}else{
        ...
        $file[$no]['view'] = view_file_link($row['bf_file'], $row['bf_width'], $row['bf_height'], $file[$no]['content'], $row['bf_source']);
...
function conv_content($content, $html, $filter=true)
{
	...
	//echo "====>[   $board[bo_use_dhtml_editor]    ]";
    ...
    if ($html)
    {
		...
		// 테그 제거 필터, $board[bo_use_dhtml_editor] 추가
        if($filter && $board[bo_use_dhtml_editor] == '1'){
            $content = html_purifier($content);
		}

		//echo "텍스트 $content"; exit;
...
function get_member($mb_id, $fields='*')
{
	...
	$row = sql_fetch(" select $fields from {$g5['member_table']} where mb_id = TRIM('$mb_id') ");

	$chaptercode = $row[chaptercode];
	$joblevel = $row[joblevel];

	$arr_chaptercode = get_jibu_array();
	$arr_joblevel = get_joblevel_array();

	$row[chaptercode_name] = $arr_chaptercode[$chaptercode];
	$row[joblevel_name] = $arr_joblevel[$joblevel];
...
function view_file_link($file, $width, $height, $content='', $bf_source='')
{
    global $config, $board;
    global $g5;
    static $ids;
    ...
    if (preg_match("/\.({$config['cf_image_extension']})$/i", $file)) {
		...
		// 이전한 DB 라면
		if( $bf_source == $file ){
			$img = '<a href="'.G5_BBS_URL.'/view_image.php?bo_table='.$board['bo_table'].'&amp;fn='.$file.'&amp;bf_source='.$bf_source.'" target="_blank" class="view_image">';
			$img .= '<img src="'.G5_DATA_URL.'/upload/'.$file.'" alt="'.$content.'" '.$attr.'>';
		}else{
...
function get_browscap_info($agent)
{
 ...
 // array 인코딩
function iconv_array($s1, $s2, $arr) {
    while (list($key, $val) = each($arr)) {
		...
		// 1 바이트 짤리면 conv 안됨
		if( !$val && $val2 ){
			$val = substr($val2, 0, (strlen($val2)-1) );
			$val = iconv($s1, $s2, $val);
		}
        $arr[$key] = $val;
    }
...
// 지부 배열 리턴
function get_jibu_array(){
	$ARR = array();
    $sql = " SELECT * FROM btj_chapter ORDER BY cvalue  ";
	$result = sql_query($sql);
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		$ARR[$row['code']] = $row['cvalue'];
    }
    return $ARR;
}
// 지부코드 SELECT
function get_jibu_select($name, $selected='', $event=''){
	$str = "<select id=\"$name\" name=\"$name\" $event>\n";
	$str .= "<option value=\"\">:: 지부선택 ::</option>";
	foreach( get_jibu_array() as $key => $val){
        $str .= option_selected($key, $selected, $val);
    }
    $str .= "</select>";
    return $str;
}
...
// 잡레벨 배열 리턴
function get_joblevel_array(){
	$ARR = array();
    $sql = " SELECT * FROM btj_joblevel ORDER BY num  ";
	$result = sql_query($sql);
    for ($i=0; $row=sql_fetch_array($result); $i++) {
		$ARR[$row['code']] = $row['cvalue'];
    }
...
// 잡레벨 SELECT
function get_joblevel_select($name, $selected='', $event=''){
	$str = "<select id=\"$name\" name=\"$name\" $event>\n";
	$str .= "<option value=\"\">:: 잡레벨선택 ::</option>";
	foreach( get_joblevel_array() as $key => $val){
        $str .= option_selected($key, $selected, $val);
    }
    $str .= "</select>";
    return $str;
